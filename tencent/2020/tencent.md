...
- 实时协同编辑的挑战 Websocket
  ...
  - 房间管理
    ...
    - 数据大时，比如复制粘贴10w条数据，除了考虑像 Websocket 这种需要自行进行数据压缩（HTTP 本身支持压缩）以外，我们还需要实现自己的分片逻辑。当涉及数据分片之后，紧接而来的还有如何分片、分片数据丢失的一些情况处理。
  - 富文本
    - 一个简单的 div 增加 contenteditable属性，用浏览器原生的 execCommand执行
